...
import { predictEthUp5m } from "./strategy/predictor.js";
...
async function run(): Promise<void> {
  ...
  while (true) {
    try {
      const markets = await gamma.getMarkets(500);
      ...
      const pred = predictEthUp5m(closes);
      const decision = makeDecision(pred, best, cfg);

      log("prediction", pred);
      log("decision", decision);

      if (decision.action === "SKIP" || !decision.tokenId || !decision.limitPrice || !decision.shareSize) {
        await sleep(cfg.pollIntervalSec * 1000);
        continue;
      }
      ...
      state.markTraded(best.marketId);
    } catch (err) {
