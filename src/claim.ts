...
async function pickWorkingRpcUrl(rpcUrls: string[], chainId: number): Promise<string> {
  ...
  for (const url of rpcUrls) {
    ...
    try {
      ...
      console.log("[claim] rpc connected", { rpcUrl: url });
      return url;
    } catch (err) {
      tried.push({ url, error: getErrorMessage(err) });
      console.log("[claim] rpc failed", { rpcUrl: url, error: tried[tried.length - 1].error });
    }
  }
...
async function executeRedeem(
  ...
  try {
    ...
    if (txType === RelayerTxType.SAFE && /safe not deployed/i.test(message)) {
      console.log("[claim] safe not deployed; deploying now");
      ...
      console.log("[claim] safe deployed", {
...
async function main(): Promise<void> {
  ...
  const whitelist = new Set(cliIds.map((x) => x.toLowerCase()));
  ...
  if (conditionIds.length === 0) {
    console.log("[claim] no redeemable condition ids found", { user, redeemablePositions: redeemable.length });
    return;
  }

  if (cfg.funderAddress && cfg.funderAddress.toLowerCase() !== signer.address.toLowerCase()) {
    console.log("[claim] info: FUNDER_ADDRESS differs from signer address; relayer flow will target signer's proxy/safe path", {
      signer: signer.address,
      funder: cfg.funderAddress,
    });
  }

  console.log("[claim] targets", {
    user,
  ...
  if (cfg.dryRun) {
    console.log("[claim] dry-run conditionIds", conditionIds);
    return;
  }

  const rpcUrl = await pickWorkingRpcUrl(cfg.rpcUrls, cfg.chainId);
  ...
  for (const conditionId of conditionIds) {
    try {
      ...
      await executeRedeem(client, txType, tx, conditionId);
      success += 1;
    } catch (err) {
      failed += 1;
